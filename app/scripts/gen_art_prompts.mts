...
const SUIT_HUE: Record<string, string> = { 威: "朱红", 理: "靛青", 利: "赭金", 情: "藕荷" };
const SUIT_EMBLEM: Record<string, string> = { 威: "官印/虎符", 理: "卷宗/天平", 利: "秤/铜钱", 情: "同心结/琴" };
...
md += `> **甲·去字化**：卡面主体=每张独立插画；**威/理/利/情 四字绝不印在卡面上**，仅作「色相 + 非字纹章」由 UI 叠加（详见第 3 节）。根治跨剧本"满屏四个字"的频率感。\n`;
...
md += `## 2. 负向提示词（所有图通用）\n\n\`\`\`\n${NEG}\n\`\`\`\n\n`;
md += `## 3. 甲·花色色相 + 非字纹章（卡面零汉字四字）\n\n`;
md += `| 花色 | 色相 | 纹章（符号，非汉字） |\n|---|---|---|\n| 威 | ${SUIT_HUE["威"]} | ${SUIT_EMBLEM["威"]} |\n| 理 | ${SUIT_HUE["理"]} | ${SUIT_EMBLEM["理"]} |\n| 利 | ${SUIT_HUE["利"]} | ${SUIT_EMBLEM["利"]} |\n| 情 | ${SUIT_HUE["情"]} | ${SUIT_EMBLEM["情"]} |\n\n`;
md += `> 卡面**不生成**任何威/理/利/情 汉字；纹章与色相由 UI 在卡框上叠加（接入阶段用 SVG/描边实现，非生图内容）。\n\n`;
md += `## 4. 乙·门类词汇表（卡面主类目）\n\n| 门类 | 释义 |\n|---|---|\n| 刑名 | 司法·案件·律法·刑讯 |\n| 盐铁 | 织造·丝绸·矿冶·垄断货物 |\n| 漕运 | 水运·舟船·河工 |\n| 科举 | 考场·士子·功名·文牍 |\n| 闺阁 | 家眷·亲情·女性·私语 |\n| 宫闱 | 朝堂·内廷·官印·密信·太监 |\n| 江湖 | 市井·侠隐·暗流·底层 |\n| 僧道 | 方外·寺观（预留） |\n| 边军 | 军伍·亲兵·武备 |\n| 商贾 | 银钱·买卖·市集·账目 |\n| 药石 | 毒药·医理·丹膏·化学 |\n| 琴棋 | 诗书画·雅器·文心 |\n| 工造 | 器物·百工·杂件 |\n\n`;
md += `> 卡面主类目标签显示「门类」而非四字；映射见 \`src/data/cardThemes.ts\`（104 张全覆盖）。\n\n`;
md += `## 5. 三档稀有度边框\n\n| 稀有度 | 边框处理 |\n|---|---|\n| 凡 | ${RARITY_BORDER["凡"]} |\n| 良 | ${RARITY_BORDER["良"]} |\n| 孤品 | ${RARITY_BORDER["孤品"]} |\n\n`;
...
md += `- \`App.tsx\` 卡牌渲染：\`.play-card\` 内 \`<img class="card-art">\` 占满卡面主体；叠加三层框：① \`.suit-seal\`（按花色色相 + 非字纹章 SVG，\**绝不显示威/理/利/情 汉字\**）；② \`.theme-tag\`（门类文字标签，来自 \`cardThemes[id]\`）；③ \`.rarity-border\` 极细边框。\n`;
